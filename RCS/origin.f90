...
contains
  ...
  ! 计算Rn_phi 时需要使用的被积函数
  ! ----------------------------------
  function genRnphi(phi)
    implicit none
    complex*16 :: genRnphi(n)
    real*8 :: phi
    ! integer*4 :: i
    ...
    do rows = 1, n, 1
      phi_n = 2.0d0*pi*rows/n-delta/2.0d0
      l = phi_n - delta/2.0d0
      u = phi_n + delta/2.0d0
      genRnphi(rows) = Gauss1d_(l, u, phi)
    end do
  end function genRnphi
  ...
  function Gauss1d_(l, u, phi) result(gauss)
    implicit none
    complex*16 :: gauss
    integer*4 :: i
    real*8 :: l, u, phi
...
program main
  ...
  implicit none
  integer*4   :: i, ii
  complex*16  :: zmn(n, n), rb(n), an(n), x0(n), rn(n)
  real*8      :: start, finish
  complex*16 :: temp1
  real*8 :: sigma
  real*8 :: phi
  ...
  do ii = 1, 360, 1
    temp1 = (0.0d0, 0.0d0)
    phi = 2.0d0*(ii-180.0d0)*pi/360
    rn = genRnphi(phi)
    do i = 1, n, 1
      temp1 = temp1 + b*rn(i)*an(i)**2
    end do
    temp1 = temp1*conjg(temp1)
    sigma = k**2*ita**2*temp1/(4.0d0*pi)
    sigma = 10.0d0*log10(sigma)
    write(unit=11, fmt=*) ii, sigma
  end do
